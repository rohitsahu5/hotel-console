...
export class Details extends React.Component {
    ...
    getDetails = () => {
        db.ref("complaints/" + this.props.match.params.id).once("value").then(res => {
            ...
            return storage.child("Complaint-" + this.props.match.params.id + "." + this.state.rDetails.imgExt).getDownloadURL()
            ...
            var newDetails = this.state.rDetails;
            for (var i = 0; i < newDetails.ComplaintTopic.length - 1; i++) {
                newDetails.ComplaintTopic[i] += ", "
            }
            for (i = 0; i < newDetails.Likings.length - 1; i++) {
                newDetails.Likings[i] += ", "
            }
        ...
        if (this.state.loaded) {
            return (
                <div className="details">
                    ...
                    <img className="col center s10 m4 display-image" onClick={() => window.location = this.state.imgURL} alt="Open this Image" src={this.state.imgURL} />
